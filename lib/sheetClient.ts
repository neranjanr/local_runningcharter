...
export function getSheetSettings(): { sheetId: string; scriptUrl: string } {
  if (typeof window === 'undefined') return { sheetId: '', scriptUrl: '' };
  return {
    sheetId: localStorage.getItem('mobile.sheetId') || process.env.NEXT_PUBLIC_SHEET_ID || '',
    scriptUrl: localStorage.getItem('mobile.scriptUrl') || process.env.NEXT_PUBLIC_SCRIPT_URL || '',
  };
}
