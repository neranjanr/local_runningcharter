...
**Quick Places (Home ↔ Office)**: Two convenience pill buttons shown directly beside the `Places Visited` field on `New Trip Entry` (`components/QuickTripForm.tsx:481`), **All Trips → Insert After** (`components/dashboard/AllTripsMasterTable.tsx:2052` `insert-quick-places-*`, same `Home - Office` / `Office - Home` pills + editable-after-fill) and Mobile (`components/mobile/MobileQuickTripForm.tsx:178`, `quicktrip-mobile/dist/mobile_app_public.html:93`, `/mobile` route): `Home - Office` and `Office - Home`. Pressing a pill clears the textbox and fills it with the pill text; the field remains freely editable afterward (`setPlacesVisited(v)` + focus, `data-testid="quick-places-home-office"` / `quick-places-office-home"` / `insert-quick-places-*`). Static PWA template mirrors the same JS (`setPlacesQuick` + `places input` sync) so the private phone file inherits the chips after `npm run build:mobile`.
...
**Estimated Start Time**: Suggested Start Time derived as End Time − (Distance / speed) ceiled to the nearest 5 minutes where speed is resolved from configurable **Speed Slab**s (see below); auto-filled only when Start Time is empty and editable, with `+5/+10/−5/−10` nudge buttons to adjust (`formatMinutesToTime` wrap, `calculateDurationMinutes` refresh) on both `New Trip Entry` (`components/QuickTripForm.tsx:98,113,210`) and **All Trips → Insert After** (`components/dashboard/AllTripsMasterTable.tsx:603` `maybeAutoEstimateInsert` + `nudgeInsertStartTime` + `Auto`, same ceiled 5-min + Traffic-aware slab resolution). During Excel Workbook import (`parseAllTripsWorkbook`) and Buffer Sheet Pull (`toPartial`), an empty `start_time` is estimated from `end_time` + `distance` using the **Traffic** set when **Traffic Mode** dual-set is active (fallback) and persisted as `HH:MM` (overwrite on same Odo Key); if `end_time` is empty the row is rejected; if estimation yields `null` (zero/invalid distance) the Trip keeps `start_time = ""` and still imports. Help page renders the active slab table dynamically.
...
**Traffic Mode**: Transient selector `Traffic` (default) vs `Light Traffic` shown only when **Speed Slab** mode is `traffic-aware`; appears in Add-Trip forms (`components/QuickTripForm.tsx`, `quicktrip-mobile/dist/mobile_app_public.html`) and **All Trips → Insert After** (`components/dashboard/AllTripsMasterTable.tsx:603`) beside Trip Type and controls which Speed Slab set the `Auto`/auto-estimate uses; **Start Time Audit** (`components/dashboard/AllTripsMasterTable.tsx:150` `handleOpenStartAudit`) also uses the Traffic set when `mode==='traffic'` (otherwise active slab). Not persisted to `Trip` table/DB nor Excel/Sheet export — evaluation helper only.

**Start Time Anomaly**: A Trip where `start_time` is anomalous against `end_time`+`distance` via the **Estimated Start Time** Traffic engine: `Empty` (`start==""` estimatable), `Equal` (`start==end` zero-duration), `Inverted` (`parse(start) > parse(end)` wall-clock inverted, short overnight <6h exempted), or `Short` (`actualDuration < estimatedDuration` i.e. `start > estimated_start` where `estimated_start = end − ceil(distance/speed*60/5)*5` using Traffic slabs).
_Avoid_: Bad time, wrong start

**Start Time Audit**: Read-only header action `⏱️ Verify Start Times` in All Trips Master Table that scans the entire Book for **Start Time Anomalies**, lists them chronologically (`Date | KM | Reason | Start | End | Est(Traffic) | Duration→Est (+Δ)`) with per-row checkboxes (disabled when `est==null`), Select All / by-reason, and bulk `Fix Selected → Estimated` that overwrites `start_time` via `updateTrip` and focuses the earliest fixed row.
_Avoid_: Time checker, start fix
...
**Inserted Trip (Shift)**: A Trip inserted after any existing Trip at a chosen chronological position in All Trips via the Insert After dialog (`components/dashboard/AllTripsMasterTable.tsx:603`); dialog provides ODO reciprocal fields `Start KM + Distance = End` (any of the three editable, Integer KM), **Estimated Start Time** auto-fill + `Auto` + `−10/−5/+5/+10` nudges bound to `End Time`/`Distance` via the active **Speed Slab**/**Traffic Mode** set (same engine as New Trip), and **Quick Places** pills `Home - Office` / `Office - Home`; by default downstream Trips (all later in `date || start_km` order, across Page boundaries) have their `Start KM` and `End KM` increased uniformly by `Δ = new.end − new.start` (their `trip_distance` and other fields frozen), with `BookPage` start/end KM and fuel chain recomputed forward via `recalculatePageBalancesFromOpening`; pagination auto-splits (`13 trips/day`, `4 days/page`, month rollover) creating a new Page when needed. When downstream contains an **ODO Gap Period**, the dialog offers two flavours (see below). Confirmation previews `N` shifted trips and `Δ` before save.
_Avoid_: Add record in between, middle insert

**Gap-Absorbed Insert (Bounded Shift)**: Variant of **Inserted Trip (Shift)** offered only when a downstream **KM Gap** exists; the next ODO gap vacuum `G = successor.start_km − predecessor.end_km` after the insertion point absorbs the new Trip's `Δ`. All Trips strictly between insertion point and gap successor shift by `Δ`, gap extent reduces to `G' = max(0, G − Δ)` with spill `S = max(0, Δ − G)` shifting Trips beyond the gap; when `Δ ≤ G` no Trip beyond the gap moves and the gap shrinks (or vanishes if `Δ = G`).
_Avoid_: Fill-to-gap insert, partial shift

**Gap-Preserving Shift (Unbounded)**: Variant of **Inserted Trip (Shift)** where all downstream Trips shift uniformly by full `Δ` and any downstream **KM Gap** vacuum is preserved/moved intact.
_Avoid_: Shift-all

**Reverse Gap Fill**: Global reconciliation operation that compresses **KM Gap** vacuums backward so the Book's recorded Final ODO matches the physical odometer; user enters either Physical ODO or Reverse Shift Δ (reciprocal Integer KM: `Δ = recordedFinal − physicalOdo`), validated `0 < Δ ≤ totalPositiveGapExtent` (reject when `Δ > total` with max-compressible hint `recordedFinal − totalGaps` or when `physicalOdo > recordedFinal`), then earliest-first sequential absorption `G1→max(0,G1−Δ)`, spill `S=max(0,Δ−G1)` to `G2...` shifting the tail segment of each affected gap backward by its consumed share (`trip_distance` frozen), recomputing `BookPage` KM and fuel chain from **Book Opening** (`recalculatePageBalancesFromOpening`) with a `⚠️ Fuel Position & Closing Balances will be recalculated` warning; surfaced as a small `↩ Reverse Gap Fill` button inside the **Continuity Alert** banner only when positive KM Gaps exist (disabled when overlaps `extent<0`, hidden for fuel-only gaps), with a per-gap preview `G before→after` and totals before atomic `fleetledger:data-changed` commit.
_Avoid_: Back Fill Gaps, backfill, ODO adjust, reverse shift (bare)
